...
export async function notifyNewToken(a: NewTokenAlert, verdict: Verdict | null = null): Promise<void> {
  ...
  await send(
    [
      ...
      `<i>detected in real time from the sequencer — DexScreener may not have indexed it yet</i>`,
      pre(T.join("\n")),
      ...radarLines(verdict),
      `<code>${a.token}</code>`,
    ].join("\n"),
...
export async function notifyRebalance(i: RebalanceInfo): Promise<void> {
  await send(
    [
      ...
      `OOR position #${i.oldTokenId} closed → reopened as #${i.newTokenId ?? "?"}, recentered at the current price.`,
      `<i>capital is back in range, earning fees again. Check /list</i>`,
    ].join("\n"),
  );
}
...
export async function notifyCompound(i: CompoundInfo): Promise<void> {
  await send(
    [
      ...
      `fees of ~$${i.feeUsd.toFixed(2)} harvested & added back to the position (auto-compound).`,
      `<i>Check /list</i>`,
    ].join("\n"),
  );
}
...
export async function notifyOutOfRange(a: OutOfRangeAlert): Promise<void> {
  ...
  await send(
    [
      ...
      `Price moved <b>${a.side === "atas" ? "above" : "below"}</b> the range — the position has stopped earning fees.`,
