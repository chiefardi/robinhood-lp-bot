...
export async function maybeAutoLp(candidate: Candidate, verdict: Verdict | null): Promise<AutoLpResult | null> {
  ...
  if (inOorCooldown(candidate.token)) return skip(`OOR cooldown ${oorCooldownLeftMin(candidate.token)}m (repeated open-close cycles)`);
  ...
  if (a.requireGmgn && !g) return skip("GMGN required but unavailable");
  if (g) {
  ...
  if (held || justOpened) return skip(`position already exists for ${candidate.symbol} — 1 token = 1 position`);
  ...
  if (lastHour >= a.maxPerHour) return skip(`${lastHour} opens/hour ≥ maxPerHour ${a.maxPerHour}`);
  ...
  if (!acquireWallet()) return skip("wallet is sending another tx (serialized to prevent nonce collisions)");
  try {
