...
async function simulateAndSend(calldata: string, value: string, label: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(`v4 ${label} simulation reverted: ${((e as any).shortMessage || (e as Error).message || "").slice(0, 140)}`);
  }
...
export async function closeV4Position(tokenId: string, reason?: "TP" | "SL" | "OOR" | "VFADE" | "FVLOW" | "manual"): Promise<V4CloseResult> {
  ...
  try {
    ...
    log.warn(`force-close #${tokenId}: forfeited ${forfeited} (token blocks transfers/honeypot), ETH recovered`);
  }
  ...
  if (valuationBroken) log.warn(`#${tokenId} ${pair}: invalid valuation (out=${outEth} fee=${feeEth} basis=${basisEth}) → pnl recorded as null, legs zeroed`);
...
export async function compoundV4Position(tokenId: string): Promise<V4CompoundResult> {
  ...
  if (c0.toLowerCase() === NATIVE || c1.toLowerCase() === NATIVE) {
    return { compounded: false, reason: "ETH pair (compound supports USDG/ERC20 pairs only)" };
  }
  ...
  if (fee0 <= 0n && fee1 <= 0n) return { compounded: false, reason: "no accrued fees" };
  ...
  if (position.liquidity.toString() === "0") return { compounded: false, reason: "fees too small/unbalanced to add liquidity" };
