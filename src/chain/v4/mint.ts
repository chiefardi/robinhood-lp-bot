...
export async function openV4SingleSide(
  ...
  if (!pool) throw new Error("no v4/ETH pool with liquidity");
  ...
  try {
    ...
    throw new Error(`v4 mint simulation reverted: ${((e as any).shortMessage || (e as Error).message || "").slice(0, 140)}`);
  }
...
async function sweepLeftoverToEth(sides: Array<{ addr: string; dec: number }>): Promise<string | undefined> {
  ...
  for (const { addr, dec } of sides) {
    ...
    try {
      ...
      if (k?.tx) {
        hash = k.tx;
        log.info(`sweep remaining ${ethers.formatUnits(raw, dec)} ${a === USDG.toLowerCase() ? "USDG" : "token"} → ${ethers.formatEther(k.amountOut)} ETH`);
      }
    } catch {
...
export async function openV4InRange(
  ...
  if (ethToSwap >= ethers.parseEther("0.00002")) {
    ...
    if (kyberEnabled()) {
      ...
      if (k && k.amountOut > 0n) {
        swapHash = k.tx;
        out = k.amountOut;
        log.info(`buy ${meta.symbol} via KyberSwap (best route) → ${out}`);
      }
    }
    if (out <= 0n) {
      const via = (await bestSwapPool(pools, ethToSwap)) ?? pool;
      const sw = await swapEthToTokenV4(via.poolKey, ethToSwap);
      if (sw.amountOut <= 0n) throw new Error("ETH→token swap failed (pool depleted?)");
      swapHash = sw.tx;
    }
  ...
  if (tokenBal <= 0n) throw new Error("token balance 0 — nothing to provide as LP");
  ...
  try {
    ...
    throw new Error(`v4 in-range mint simulation reverted: ${((e as any).shortMessage || (e as Error).message || "").slice(0, 140)}`);
  }
  const tx = await w.sendTransaction({ to: C.v4PositionManager!, data: calldata, value: BigInt(value), ...(await overrides()) });
  const rc = await waitTx(tx, "v4-mint");
  const tokenId = tokenIdFromReceipt(rc!);
...
export async function increaseV4Position(tokenId: string, amountEthStr: string): Promise<V4OpenResult & { swapHash?: string; swappedPct: number }> {
  ...
  if (!usdgIs) throw new Error("increasing ETH-pair liquidity is not supported yet — USDG pairs only (close & reopen ETH pairs).");
  const poolId = computePoolId(poolKey);
  const sv = new ethers.Contract(C.v4StateView!, STATEVIEW_ABI, provider);
  const s0 = await sv.getSlot0!(poolId);
  if (!(s0.sqrtPriceX96 > 0n)) throw new Error("could not read this position's pool state");
