...
test('successful native-output dust refund is not mistaken for zero output when gas exceeds proceeds',async()=>{
 const f=fixture('dust');const r=await f.api.kyberSwap(TOKEN,NATIVE,10n,f.strict);
 assert.equal(r.amountOut,1n);assert.equal(f.calls.send,1);
});
