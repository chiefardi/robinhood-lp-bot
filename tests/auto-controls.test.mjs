...
test('English controls keep entry pause separate from stopping exits',async(t)=>{
  ...
  let starts=0,stops=0;const messages=[];const d={store:s,persist:()=>{},start:()=>{starts++},stop:()=>{stops++},send:async m=>messages.push(m),walletBusy:()=>false};
...
test('changing protection pauses entries and cannot silently continue without a hard SL',async(t)=>{
  ...
  const d={store:s,persist:()=>{},start:()=>{},stop:()=>{},send:async()=>{},walletBusy:()=>false};
