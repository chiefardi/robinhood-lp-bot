...
for(const failure of ['', 'token-route', 'cost', 'mode-change'])test(`asymmetric auto entry ${failure||'success'} checks both legs and preserves exact mode`,async()=>{
 ...
 const session={paused:false,lossTriggered:false,entries:[]};let reserves=0,commits=0,snapshotCount=0,mints=0;const funding=[];
 const riskStore={entryAllowed:()=>true,openPositions:()=>[],snapshot:()=>session,reserveEntry:()=>{reserves++;session.entries.push({id:'r',status:'reserved'});return 'r'},failEntry:()=>{},commitEntry:()=>{commits++}};
 ...
 assert.equal(result.opened,!failure,result.reason);
 assert.equal(mints,failure?0:1);assert.equal(commits,failure?0:1);
