...
2. For those tokens, inspect DexScreener Robinhood v4 pools. Prioritize tokens by the strongest qualifying pool's **exact-pool** five-minute volume, requiring valid five-minute and one-hour volume and both buys and sells. Keep existing 24-hour pool volume/fee and liquidity prefilters. Limit expensive on-chain `qualifyCandidate` calls to the top 20. Re-evaluate the exact pool chosen by `qualifyCandidate`; only a hook-free USDG pool with 3–5% fee can reach entry.
   Apply a USDG quote filter during qualification and at funded preflight so a busier ETH pool cannot displace an otherwise eligible USDG pool.
